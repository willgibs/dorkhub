...
/** The three steps, each with a decorative (aria-hidden) UI vignette. */
const STEPS = [
...
/**
 * The signed-out home's closing section (U2): three connected steps, the
 * is/isn't panel folded in beneath them (board: expectation-setting belongs
 * inside the story, not as its own symmetric block), then the conversion
 * capture — a display-scale line and the CTA into `/new`, which streams
 * signed-out visitors through `/auth/signin?next=/new` and back.
 */
