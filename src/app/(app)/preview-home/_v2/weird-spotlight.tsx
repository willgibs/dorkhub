 ...
 * 0023), given a bespoke stage: halftone backdrop, display-scale title, and
 * the serendipity CTA pointing at the true-random /weird. Absence rule: no
 * pick, no section.
 */
export function WeirdSpotlight({ row }: { row: FeedRow | null }) {
  ...
  return (
    <article className="edge-highlight relative flex flex-col overflow-hidden rounded-lg border bg-card sm:flex-row">
      <div
        aria-hidden="true"
        className="bg-halftone pointer-events-none absolute inset-0 opacity-40 [mask-image:radial-gradient(420px_260px_at_18%_0%,black,transparent_70%)] [-webkit-mask-image:radial-gradient(420px_260px_at_18%_0%,black,transparent_70%)]"
      />

      <div className="relative flex min-w-0 flex-1 flex-col gap-3.5 px-6 py-6">
        <p className="font-mono text-[11.5px] uppercase tracking-widest text-primary">
          <span aria-hidden="true">{'// '}</span>
          {copy.weirdSpotKicker}
        </p>

        <h3 className="font-display text-[26px] font-extrabold leading-tight tracking-tight sm:text-3xl">
        ...
        <p className="tabular-nums flex flex-wrap items-center gap-3.5 font-mono text-xs text-muted-foreground">
          <LanguageDot
      ...
      <div className="relative aspect-[2/1] shrink-0 border-t bg-surface-2 sm:w-[320px] sm:border-t-0 sm:border-l sm:[aspect-ratio:auto]">
        {row.repo_full_name ? (
