...
export async function getRelatedProjects(
  ...
  const cached = unstable_cache(
    ...
    ['related', projectId],
    // 24h, matching the project page above it. A route's effective
    // revalidate is the MINIMUM across every cache it reads, so a shorter
    // window here would silently cap all 16,972 project pages (the trap
    // that bit three times on 2026-08-03). "More like this" moves at sync
    // cadence like everything else.
    { revalidate: 86400, tags: ['related'] },
  );
