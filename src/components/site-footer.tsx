...
export type SiteFooterProps = {
  /**
   * Live platform counts, when the surface has them. Presentational by
   * contract (components/ never fetch) — the route groups' layouts pass the
   * cached `getPlatformStats()`; surfaces that shouldn't pay for a query to
   * render (404, styleguide demos) simply omit it and the line is absent.
   */
  stats?: PlatformStats | null;
};

/**
 * Site footer (U2): the one-line footer grew into the platform's closing
 ...
 */
export function SiteFooter({ stats = null }: SiteFooterProps) {
